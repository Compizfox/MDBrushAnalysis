...
class CrosslinkGenerator(BrushGenerator):
    """
    	Generate a LAMMPS data file containing a Kremer-Grest polymer brush grafted to a planar wall in a rectangular box, adding single-particle side chains and/or backbone heteroparticles.
    	Defaults are equivalent to KremerGrestBrushGenerator. Doesn't contain a lot of checks, so silly inputs will yield silly and unphysical data files. You have been warned.
    ...
    def __init__(self, box_size: Tuple[float, float, float], rng_seed: Optional[int], side_freq: float=0.0, het_freq: float=0.0):
        """
        ...
        :param float side_freq: Chance of a given backbone particle having a side group attached to it, mutually exclusive with being a heteroparticle.
        :param float het_freq: Chance of a given backbone particle being a heteroparticle, mutually exclusive with having a side group.
        """
        bead_size = 1  # (sigma)
